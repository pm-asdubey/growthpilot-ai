      ...
      {checks.map((check) => (
        <li key={check.label} className="flex items-start gap-3">
          <span className="mt-0.5 shrink-0" aria-hidden="true">
            {check.status === 'pending' && <Circle size={16} className="text-muted-foreground" />}
            {check.status === 'pass' && <CheckCircle size={16} className="text-success" />}
            {check.status === 'fail' && <XCircle size={16} className="text-error" />}
            {check.status === 'warn' && <AlertTriangle size={16} className="text-warning" />}
          </span>
          <span className={cn(
            ...
            {check.label}
            {check.message && <span className="ml-1 text-muted-foreground">— {check.message}</span>}
          </span>
        </li>
      ))}
    </ul>
