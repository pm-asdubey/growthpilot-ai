...
function buildTrendData(leads: Lead[]): ConversionTrendData[] {
  return DAY_BUCKETS.map(({ label, min, max }) => {
    const bucket = leads.filter((l) => {
      const raw = l.days_since_signup
      const n: number = typeof raw === 'boolean' ? (raw ? 1 : 0) : raw
      return n >= min && n <= max
    })
    const converted = bucket.filter((l) => l.converted).length
    return { bucket: label, converted, notConverted: bucket.length - converted }
  }).filter((b) => b.converted + b.notConverted > 0)
}
